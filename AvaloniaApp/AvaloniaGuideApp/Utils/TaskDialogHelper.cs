using Avalonia.Controls;
using Avalonia.Media;
using FluentAvalonia.UI.Controls;
using System.Threading.Tasks;

namespace AvaloniaGuideApp.Utils
{
    public class TaskDialogHelper
    {
        public static async Task ShowTaskDialogAsync(Window owner, string header, string content, TaskDialogType dialogType)
        {

            var taskDialog = new FATaskDialog
            {
                Header = header,
                Content = content,
                FooterVisibility = FATaskDialogFooterVisibility.Never,
                IsFooterExpanded = false,
                ShowProgressBar = false,
                XamlRoot = owner
            };

            taskDialog.Buttons.Add(new FATaskDialogButton { Text = "OK", DialogResult = "OK" });

            switch (dialogType)
            {
                case TaskDialogType.Warning:
                    taskDialog.IconSource = new FASymbolIconSource { Symbol = FASymbol.AlertOn, Foreground = Brushes.Orange };
                    taskDialog.Title = "Warning";
                    break;
                case TaskDialogType.Error:
                    taskDialog.IconSource = new FASymbolIconSource { Symbol = FASymbol.Clear, Foreground = Brushes.Red };
                    taskDialog.Title = "Error";
                    break;
                case TaskDialogType.Success:
                    taskDialog.IconSource = new FASymbolIconSource { Symbol = FASymbol.Accept, Foreground = Brushes.Green };
                    taskDialog.Title = "Success";
                    break;
                case TaskDialogType.Information:
                    taskDialog.IconSource = new FASymbolIconSource { Symbol = FASymbol.Important, Foreground = Brushes.Blue };
                    taskDialog.Title = "Information";
                    break;
            }

            await taskDialog.ShowAsync(true);
        }

        public static async Task<bool> ShowQuestionDialogAsync(Window owner, string header, string content)
        {

            var taskDialog = new FATaskDialog
            {
                Title = "Question",
                Header = header,
                Content = content,
                FooterVisibility = FATaskDialogFooterVisibility.Never,
                IsFooterExpanded = false,
                ShowProgressBar = false,
                IconSource = new FASymbolIconSource { Symbol = FASymbol.Help, Foreground = Brushes.BlueViolet },
                XamlRoot = owner
            };

            var yesButton = new FATaskDialogButton { Text = "Yes", DialogResult = "Yes" };
            var noButton = new FATaskDialogButton { Text = "No", DialogResult = "No" };

            taskDialog.Buttons.Add(yesButton);
            taskDialog.Buttons.Add(noButton);
            var result = await taskDialog.ShowAsync(true);

            return result is "Yes";
        }
    }

    public enum TaskDialogType
    {
        Warning,
        Error,
        Success,
        Information
    }
}
